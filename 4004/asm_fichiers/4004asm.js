	...
	while (sym) {
		...
		if ((c1<'A') || (c1>'Z')) {
			...
			if (typeof par1=='undefined') {
				...
				if(pass==1) {
					listing.value+=par1;
					if ((mode>3) && (mode<7)) listing.value+=' '+par2;
				}
				if(pass==2) {
...
function getHexWord(v) {
	return ''+hextab[Math.floor(v/0x0100)]+hextab[Math.floor((v&0xf0)/16)]+hextab[v&0x00f]
}
